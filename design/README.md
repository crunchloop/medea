...
| [`cluster-bootstrap.md`](cluster-bootstrap.md) | **(Phase B, Draft for review)** Medea creates a new single-control-plane cluster from bare metal — *generates* the PKI, renders + serves the first CP config, bootstraps etcd, captures client creds — so the home-cluster rebuild is Medea-driven and `_out/` never returns. Fulfills the deferred `provisioning-plane.md` §9; builds on `credentials.md` (Phase A) + the v2 provisioning primitives. |
