...
| **Provisioning Integration** | Generic subdomain (v2) | [`provision`](internal/provision), [`provision/matchbox`](internal/provision/matchbox) | The `Provisioner` seam (Matchbox driver: group/profile/generic files), spec-based worker machine-config generation (machinery `generate` over the captured secrets bundle), and Image-Factory schematic resolution. v2-M2 building blocks; the reconciler that drives them (over `Host`) lands in v2-M3 (`design/provisioning-plane.md`). |
