...
| **Provisioning** | Domain (v2) + generic ACLs | [`provision`](internal/provision) (reconciler + ACLs), [`provision/matchbox`](internal/provision/matchbox); the `Enable/DisableProvisioning` handlers in [`server`](internal/server) | The provisioning reconciler (`Reconciler.ReconcilePool`: scale-out **and scale-in** over the `Host` aggregate — allocate → stage → bind-on-join; drain → unstage → release — gated by `Cluster.provisioning_enabled`, default off) + the `Executor` running it on an interval (`serve --provisioning`), plus the generic ACLs it composes: the `Provisioner` seam (Matchbox driver), worker machine-config generation (machinery `generate` over the captured secrets), and Image-Factory schematic resolution. Parallels Version Rollout (a guarded reconciler + executor over an aggregate). See `design/provisioning-plane.md`. |
